...
internal fun DrawScope.drawBadgeSymbol(badge: Badge, cx: Float, cy: Float, r: Float, color: Color) {
    ...
    when (badge) {
        ...
        Badge.BLITZ                 -> drawDoubleLightning(cx, cy, r, color)
        Badge.SEA_WOLF              -> drawCrosshair(cx, cy, r, color, stroke)
        Badge.LUCKY_DOG             -> drawScatterDots(cx, cy, r, color)
        Badge.COLD_OPENER           -> drawScatterDots(cx, cy, r, color)
        Badge.IRON_HULL             -> drawShield(cx, cy, r, color, stroke)
        Badge.CRUISER_LIVES         -> drawShipSilhouette(cx, cy, r, color, stroke)
        Badge.TORPEDO_ACE           -> drawWaveArc(cx, cy, r, color, stroke)
        Badge.BATTLESHIP_HUNTER     -> drawWaveArc(cx, cy, r, color, stroke)
        Badge.SMALL_GAME            -> drawShipSilhouette(cx, cy, r, color, stroke)
        Badge.SPRAY_AND_PRAY        -> drawScatterDots(cx, cy, r, color)
        Badge.NUCLEAR_OPTION        -> drawBomb(cx, cy, r, color, stroke)
        Badge.SCATTERSHOT           -> drawScatterDots(cx, cy, r, color)
        Badge.TACTICAL_RETREAT      -> drawShield(cx, cy, r, color, stroke)
        Badge.PHOENIX               -> drawFlames(cx, cy, r, color, stroke)
        Badge.SPITE                 -> drawLightning(cx, cy, r, color)
        Badge.FLEET_COMMANDER       -> drawCrown(cx, cy, r, color, stroke)
        Badge.SEA_VETERAN           -> drawSubmarine(cx, cy, r, color, stroke)
        Badge.IRON_ADMIRAL          -> drawCrown(cx, cy, r, color, stroke)
    }
}
